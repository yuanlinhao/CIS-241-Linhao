...
#includeBEEPBOOP/<complex.h>BEEPBOOP//*Oh,my_dear_friend.How_I've_missed_you.--C-3PO***/
typedefBEEPBOOP/complexBEEPBOOP/double(c);c(X)[3],P,O;cBEEPBOOP/B(double t){double s=1-t,u;P=
s*s*X[1]            +2        *s*t*        *X+t        *t*X       [2]+O;u=I*P;
return+48*((    s=P)+   48*I   )/(   1<u?   u:   1);}   /*   1977  IOCCC2020*/
#includeBEEPBOOP    Do.Or   do_not   .   There_is_   no_try...   --Yoda**/<stdio.h>
voidBEEPBOOP/b(    double   t,BEEPBOOP*   *   **/double   u){double   s=P=B(t)-B(u);(s=P
...
#includeBEEPBOOP***    your   father..   --DarthVader   **/   <time.h>BEEPBOOP**   ****/
int(main)(int    (x),   char**V){;   clock_t(c)=   /*   */clock();;;   for(d=D
;m<26;m++,s    ++)*s>   63?*d++=m%   7*          16-7   *8,*d++=m/   7*25,*d++
=*s-64:0;;    FOOPS(V[1])   {;;;FILE   *F   =fopen(V[+1],   "r");for   (d=D,L=N=m
=0;(x=BEEPBOOP             *            ***              **/            fgetc(F))>0
||fclose(F);)FOOPS(x>13?64<x&&x<91?*d++=m*16,*d++=L*25,*d++=x%26:0,m++,0:1)for(++
...
=G;for(;n--;)for(;*p++>33;);*a=a[1]=*p++;for(;*p>33;p++)FOOPS(*p%2?*a=*p,0:1){a[2
